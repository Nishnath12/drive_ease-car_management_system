...
const ApproveBookings = () => {
  ...
  const handleStatusChange = async (type, id, newStatus) => {
    try {
      ...
      setError(`Failed to update ${type} #${id}.`);
    ...
    <section className="approve-bookings section">
      <div className="section-header">
        ...
        <div className="filter-controls">
          {['all', 'pending', 'confirmed', 'cancelled'||'cancelled'].map(f => (
            <button
              key={f}
      ...
      {filteredCarBookings.length === 0 ? (
        <p>No {filter} car bookings found.</p>
      ) : (
            ...
            <tbody>
              {filteredCarBookings.map(booking => (
                <tr key={booking.id} className={`status-${booking.status}`}>
                  ...
                  <td>{booking.car_name || `Car-${booking.car_id}`}</td>
                  <td>{new Date(booking.order_date).toLocaleDateString()}</td>
                  ...
                  <td>
                    {booking.status === 'pending' && (
                      <>
                        <button onClick={() => handleStatusChange('car', booking.id, 'confirmed')} className="approve-btn">Approve</button>
                        <button onClick={() => handleStatusChange('car', booking.id, 'canceled')} className="reject-btn">Reject</button>
                      </>
                    )}
                    {booking.status === 'canceled' && (
                      <button onClick={() => handleStatusChange('car', booking.id, 'pending')} className="reconsider-btn">Reconsider</button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {filteredTestDrives.length === 0 ? (
        <p>No {filter} test drive bookings found.</p>
      ) : (
        <div className="bookings-table-container">
          <table className="bookings-table">
            <thead>
            ...
            <tbody>
              {filteredTestDrives.map(td => (
                <tr key={td.id} className={`status-${td.status}`}>
                  ...
                  <td>{td.car_name||`Car-${td.car_id}`}</td>
                  <td>{td.location_name||`Location-${td.location_id}`}</td>
                  <td>{new Date(td.preferred_date).toLocaleDateString()}</td>
                  <td>Slot {td.slot_number}</td>
                  <td>
                    <span className={`status-badge ${td.status}`}>{td.status}</span>
                  </td>
                  <td>
                    {td.status === 'pending' && (
                      <>
                        <button onClick={() => handleStatusChange('testdrive', td.id, 'confirmed')} className="approve-btn">Approve</button>
                        <button onClick={() => handleStatusChange('testdrive', td.id, 'cancelled')} className="reject-btn">Reject</button>
                      </>
                    )}
                    {td.status === 'cancelled' && (
                      <button onClick={() => handleStatusChange('testdrive', td.id, 'pending')} className="reconsider-btn">Reconsider</button>
                    )}
                  </td>
                </tr>
              ))}
