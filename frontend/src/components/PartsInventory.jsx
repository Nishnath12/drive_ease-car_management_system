...
const PartsInventory = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const partData = {
        ...
        location_id: parseInt(newPart.location_id) || 1
      };
      await SparePartsService.addSparePart(partData);
      setNewPart({ name: '', brand: '', price: '', stock_quantity: '', location_id: '' });
  ...
  const handleUpdateStock = async (id, currentStock, amount) => {
    ...
    try {
      ...
      setSpareParts(prev => prev.map(part => part.id === id ? { ...part, stock_quantity: newStock } : part));
    } catch (err) {
  ...
  if (error) {
    return (
      <div className="error-section">
        ...
        <button className="retry-btn" onClick={fetchSpareParts}>Retry</button>
        <details>
          <summary>Debug Info</summary>
          <pre>API Endpoints:\n- GET /spare-parts\n- POST /spare-parts\n- PUT /spare-parts/:id\n- DELETE /spare-parts/:id</pre>
        </details>
      </div>
    );
  }

  if (loading) return <div className="section-loading">Loading inventory...</div>;
      ...
      {formVisible && (
        <form onSubmit={handleSubmit} className="parts-form">
          <div className="form-row">
            ...
            <div className="form-group">
              <label>Brand</label>
              <input type="text" name="brand" value={newPart.brand} onChange={handleInputChange} required />
            </div>
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>Price (₹)</label>
              <input type="number" name="price" value={newPart.price} onChange={handleInputChange} min="0" step="0.01" required />
            </div>
            <div className="form-group">
              <label>Stock Quantity</label>
              <input type="number" name="stock_quantity" value={newPart.stock_quantity} onChange={handleInputChange} min="0" required />
            </div>
          </div>
          <div className="form-row">
            <div className="form-group">
              <label>Location ID</label>
              <input type="number" name="location_id" value={newPart.location_id} onChange={handleInputChange} min="1" required />
            </div>
          </div>
        ...
        {spareParts.length > 0 ? spareParts.map(part => (
          <div className="part-card" key={part.id}>
            <div className="part-info">
              ...
              {part.location_id && <p className="part-location">Location: {part.location_id}</p>}
              <div className="part-actions">
                <button className="stock-btn decrease" onClick={() => handleUpdateStock(part.id, part.stock_quantity, -1)} disabled={part.stock_quantity <= 0}>-</button>
                <button className="stock-btn increase" onClick={() => handleUpdateStock(part.id, part.stock_quantity, 1)}>+</button>
                <button className="delete-btn" onClick={() => handleDelete(part.id)}>Delete</button>
              </div>
            </div>
          </div>
        )) : (
          <div className="empty-state">
            ...
            <button className="primary-btn" onClick={() => setFormVisible(true)}>Add Your First Part</button>
          </div>
        )}
      </div>
    </section>
